    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">Manage your AI workforce across all workspaces.</p>
        </div>
        <div className="flex items-center space-x-3">
          <Link
            href="/agents/builder"
            className="bg-brand-magenta hover:brightness-110 text-white px-4 py-2 rounded-md font-medium transition-all flex items-center space-x-2"
            style={{ boxShadow: '0 0 15px rgba(233,30,140,0.2)' }}
          >
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-bg-surface p-4 rounded-xl border border-border-card">
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <button className="flex items-center justify-center space-x-2 bg-transparent border border-border-subtle rounded-md px-4 py-2 text-sm text-text-secondary hover:text-text-primary hover:border-border-card-hover transition-colors w-full sm:w-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {agents.map((agent) => (
          <div key={agent.id} className="bg-bg-surface border border-border-card hover:border-border-card-hover rounded-xl p-5 transition-all duration-200 group flex flex-col h-full relative overflow-hidden" style={{ boxShadow: 'none' }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.boxShadow = '0 0 20px rgba(0,229,204,0.1)'; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}>
            {/* Top accent glow */}
            <div className={`absolute top-0 left-0 right-0 h-px opacity-50 group-hover:opacity-100 transition-opacity ${agent.avatarBg}`}></div>
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="relative">
                ...
                <div className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-bg-base ${agent.status === 'online' ? 'bg-brand-green' : 'bg-text-muted'}`} style={agent.status === 'online' ? { boxShadow: '0 0 6px rgba(0,229,204,0.6)' } : undefined}></div>
              </div>
            ...
            <div className="mb-4 flex-1">
              ...
              <div className="flex items-center space-x-2 mt-2">
                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-bg-surface-active text-text-secondary border border-border-subtle">
                  {agent.type}
                </span>
            ...
            <div className="space-y-3 mb-5 text-sm">
              ...
              <div className="bg-bg-surface border border-border-subtle p-2.5 rounded-lg mt-2">
