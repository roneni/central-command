...
export default function AgentLogs() {
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      ...
      case 'error': return <XCircle className="w-4 h-4 text-brand-magenta" />;
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">Detailed history of all autonomous actions taken by your agents.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-bg-surface p-4 rounded-xl border border-border-card">
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <select className="bg-transparent border border-border-subtle text-text-secondary text-sm rounded-md px-3 py-2 outline-none w-full sm:w-auto focus:border-brand-cyan">
            <option>All Agents</option>
            <option>Doctor Agent</option>
            <option>Task Auto-Creator</option>
          </select>
          <button className="flex items-center justify-center space-x-2 bg-transparent border border-border-subtle rounded-md px-4 py-2 text-sm text-text-secondary hover:text-text-primary hover:border-border-card-hover transition-colors whitespace-nowrap">
      ...
      <div className="bg-bg-surface border border-border-card rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-text-muted uppercase bg-bg-surface-active/50 border-b border-border-divider">
              <tr>
              ...
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-bg-surface-hover transition-colors group">
                  <td className="px-6 py-4">
                    <div className="font-mono text-xs text-brand-cyan mb-0.5">{log.id}</div>
      ...
      <div className="flex justify-between items-center text-xs text-text-muted px-2">
        ...
        <div className="flex space-x-1">
          <button className="px-2 py-1 rounded bg-bg-surface hover:bg-bg-surface-hover border border-border-subtle transition-colors disabled:opacity-50" disabled>Previous</button>
          <button className="px-2 py-1 rounded bg-bg-surface hover:bg-bg-surface-hover border border-border-subtle transition-colors">Next</button>
        </div>
      </div>
    </div>
  );
