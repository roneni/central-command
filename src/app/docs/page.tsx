...
export default function DocsPage() {
  ...
  return (
    <div className="h-full flex bg-bg-base overflow-hidden">
      ...
      {/* Docs Sidebar */}
      <div className="w-64 border-r border-border-subtle bg-bg-sidebar flex flex-col h-full shrink-0">
        <div className="p-4 border-b border-border-subtle flex justify-between items-center sticky top-0 bg-bg-sidebar z-10">
          <h2 className="font-semibold text-text-primary flex items-center space-x-2">
            <FileText className="w-4 h-4 text-brand-cyan" />
            <span>Documents</span>
          </h2>
          <button className="p-1 text-text-secondary hover:text-text-primary bg-transparent border border-border-subtle rounded transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          {sidebarItems.map((folder, i) => (
            <div key={i} className="space-y-0.5">
              <button className="w-full flex items-center space-x-2 px-2 py-1.5 rounded-md hover:bg-[rgba(255,255,255,0.05)] transition-colors text-text-secondary group text-left">
                <ChevronRight className={`w-3.5 h-3.5 transition-transform ${folder.open ? 'rotate-90 text-brand-cyan' : 'text-text-muted'}`} />
                <Folder className={`w-4 h-4 ${folder.open ? 'text-brand-cyan' : 'text-text-muted'}`} />
                <span className="text-sm font-medium truncate flex-1">{folder.name}</span>
              </button>

              {folder.open && folder.items.map((file, j) => (
                <button key={j} onClick={() => setActiveDoc(file.name)} className={`w-full flex items-center space-x-2 pl-8 pr-2 py-1.5 rounded-md transition-colors text-left ${file.active || activeDoc === file.name ? 'bg-brand-cyan/10 text-brand-cyan border-l-2 border-brand-cyan' : 'hover:bg-[rgba(255,255,255,0.05)] text-text-muted hover:text-text-secondary'}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-bg-base relative">
        {/* Editor Topbar */}
        <div className="h-14 border-b border-border-subtle flex items-center justify-between px-6 shrink-0 sticky top-0 bg-[rgba(10,10,20,0.8)] backdrop-blur-xl z-10">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button className="flex items-center space-x-1.5 bg-transparent border border-border-subtle hover:border-border-card-hover text-text-secondary hover:text-text-primary transition-colors px-3 py-1.5 rounded-md text-sm font-medium">
              <History className="w-4 h-4" />
              <span className="hidden sm:inline">History</span>
            </button>
            <button className="flex items-center space-x-1.5 bg-brand-cyan hover:brightness-110 text-bg-base px-3 py-1.5 rounded-md font-medium transition-all text-sm">
        ...
        <div className="flex-1 overflow-y-auto w-full">
          <div className="max-w-3xl mx-auto py-12 px-8 sm:px-12 lg:px-16">
            <div className="group relative">
              <h1 className="text-4xl font-extrabold tracking-tight text-text-primary mb-6 outline-none cursor-text">
                {activeDoc}
              </h1>
            ...
            <div className="space-y-6 text-text-secondary leading-relaxed outline-none min-h-[500px] text-sm">
              ...
              <div className="bg-bg-surface border border-border-card rounded-lg p-4 font-mono text-sm text-brand-cyan my-4">
                <div className="flex items-center space-x-2 mb-2 text-text-muted border-b border-border-divider pb-2">
  ...
  return agent.execute({
    ...
    deadline: task.deadline
              ...
              <blockquote className="border-l-4 border-brand-cyan pl-4 italic bg-brand-cyan/5 py-2 pr-4 rounded-r-lg text-text-secondary">
