    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">Real-time health tracking across your digital properties.</p>
        </div>

        {/* Network Metrics Snapshot */}
        <div className="flex items-center space-x-4">
          <div className="bg-bg-surface border border-border-card rounded-lg px-4 py-2 flex items-center space-x-3">
            <div className="w-2 h-2 rounded-full bg-brand-green animate-pulse" style={{ boxShadow: '0 0 6px rgba(0,229,204,0.6)' }}></div>
            <div>
                ...
                <tr key={site.id} className="hover:bg-bg-surface-hover transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-lg bg-bg-surface border border-border-subtle flex items-center justify-center">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="flex space-x-0.5 h-3 items-end mt-1">
                      {[...Array(10)].map((_, i) => (
                        <div key={i} className={`w-1 rounded-t-sm ${i === 8 && site.status === 'error' ? 'bg-brand-magenta h-full' : 'bg-brand-green'} ${i % 2 === 0 ? 'h-[80%]' : 'h-full'}`}></div>
                      ))}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className={`text-sm font-mono ${site.status === 'error' ? 'text-brand-magenta' : 'text-text-secondary'}`}>
                        {site.responseTime}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="flex items-center space-x-1.5 bg-brand-magenta/10 hover:bg-brand-magenta/20 text-brand-magenta border border-brand-magenta/20 px-3 py-1.5 rounded-md transition-colors text-xs font-medium">
